...
Note: TypeScript module components are found in the `src/components` folder. Everything else in the working directory is scaffolding so that the browser can interpret and render content, such as "Hello React.js World!" found within `App.tsx`, as shown in the figure above, and what is expected as the output build of this very repository.
